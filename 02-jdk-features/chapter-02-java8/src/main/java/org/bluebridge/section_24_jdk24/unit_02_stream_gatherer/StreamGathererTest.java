...
/**
 * JDK 24 Stream Gatherers 测试（PREVIEW 预览特性）
 *
 * Stream Gatherers (JEP 485) 是 JDK 24 的 PREVIEW 预览特性，第三次预览，
 ...
 *       编译命令：javac --enable-preview --release 24 StreamGathererTest.java
 *       运行命令：java --enable-preview StreamGathererTest
 *
 * 演化历程: Stream Gatherer JDK 22(1st) → JDK 23(2nd) → JDK 24(JEP 485, 3rd PREVIEW) → JDK 25(STANDARD)
 *
 * @author lingwh
...
public class StreamGathererTest {
    ...
    /**
     * 测试 Gatherers.windowFixed 固定窗口分组（PREVIEW）
    ...
    public void testWindowFixed_Preview() {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        List<Integer> numbers = Stream.of(1, 2, 3, 4, 5, 6, 7, 8)
    ...
    /**
     * 测试 Gatherers.fold 自定义折叠操作（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
    ...
    /**
     * 测试 Gatherers.windowFixed 结合 map 进行分组处理（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 将流元素按 2 个一组分组，然后计算每组元素之和
     */
    @Test
    public void testWindowFixedWithMap_Preview() {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
    ...
    /**
     * 测试 Gatherers.windowSliding 计算移动平均（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 使用滑动窗口计算移动平均值
     */
    @Test
    public void testWindowSlidingMovingAverage_Preview() {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
    ...
    /**
     * 测试 Gatherers.mapConcurrent 的并发性能（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 使用 mapConcurrent 模拟耗时操作，观察并发执行效果
     */
    @Test
    public void testMapConcurrentPerformance_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview

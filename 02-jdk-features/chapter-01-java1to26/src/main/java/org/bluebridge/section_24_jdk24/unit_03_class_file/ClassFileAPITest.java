...
/**
 * JDK 24 Class-File API 测试（PREVIEW 预览特性）
 *
 * Class-File API (JEP 484) 是 JDK 24 的 PREVIEW 预览特性，第三次预览，
 ...
 *       编译命令：javac --enable-preview --release 24 ClassFileAPITest.java
 *       运行命令：java --enable-preview ClassFileAPITest
 *
 * 演化历程: Class-File API JDK 22(1st) → JDK 23(2nd) → JDK 24(JEP 484, 3rd PREVIEW) → JDK 25(STANDARD)
 *
 * @author lingwh
...
public class ClassFileAPITest {
    ...
    /**
     * 测试使用 Class-File API 解析当前类的类文件结构（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 解析当前类文件，输出类名、方法、字段等信息
     */
    @Test
    public void testParseClassFile_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        // 获取当前类的字节码
        ...
        System.out.println("  父类: " + classModel.superclass().map(ClassDesc::displayName).orElse("无"));
    ...
    /**
     * 测试使用 Class-File API 构建一个简单的类文件（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 使用 ClassFile API 构建一个简单的 HelloWorld 类
     */
    @Test
    public void testBuildClassFile_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        ClassFile classFile = ClassFile.of();

        // 构建一个简单的类: HelloWorld { public static void main(String[]); }
        byte[] classBytes = classFile.buildToBytes(ClassDesc.of("HelloWorld"), clb -> {
        ...
        try (var classBytesStream = new java.io.ByteArrayInputStream(classBytes)) {
            ...
            // 通过 URL 方式加载类
            java.lang.reflect.Method defineMethod = ClassLoader.class
                    .getDeclaredMethod("defineClass", String.class, byte[].class, int.class, int.class);
            defineMethod.setAccessible(true);
            Class<?> helloWorldClass = (Class<?>) defineMethod.invoke(loader, "HelloWorld",
    ...
    /**
     * 测试使用 Class-File API 解析和检查类的常量池信息（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 解析类文件后输出常量池相关的信息
     */
    @Test
    public void testParseClassFileConstantPool_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        String className = this.getClass().getName();
        Class<?> clazz = this.getClass();
        ...
        System.out.println("  父类: " + classModel.superclass()
                .map(ClassDesc::displayName).orElse("无"));
        ...
        for (FieldModel field : classModel.fields()) {
            ...
            // 检查是否有常量值属性
            field.findAttribute(ConstantValueAttribute.class).ifPresent(cv -> {
                System.out.println("      常量值: " + cv.constant().stringValue());
            });
        }
        System.out.println("--- 分割线 ---");
    }
    ...
    /**
     * 测试使用 Class-File API 构建带字段的类（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 构建一个包含字段、构造器和方法的完整类
     */
    @Test
    public void testBuildClassFileWithFields_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        ClassFile classFile = ClassFile.of();

        // 构建一个 Person 类: public class Person { private String name; private int age; ... }
        byte[] classBytes = classFile.buildToBytes(ClassDesc.of("Person"), clb -> {
            clb.withFlags(AccessFlag.PUBLIC);
            // 添加私有字段 name (String 类型)
            clb.withField("name", ClassDesc.of("java.lang.String"), AccessFlag.PRIVATE);
            // 添加私有字段 age (int 类型)
            clb.withField("age", ClassDesc.of("int"), AccessFlag.PRIVATE);
        ...
        defineMethod.setAccessible(true);
        ClassLoader loader = new ClassLoader() { };
        Class<?> personClass = (Class<?>) defineMethod.invoke(loader, "Person",
                classBytes, 0, classBytes.length);
    ...
    /**
     * 测试 Class-File API 解析和转换类文件（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 解析当前类，再重新构建并对比
     */
    @Test
    public void testParseAndRebuildClassFile_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        String className = this.getClass().getName();
        Class<?> clazz = this.getClass();
        ...
        // 从解析的模型中重建类文件
        byte[] rebuiltBytes = classFile.buildToBytes(classModel);
    ...
    /**
     * 测试 Class-File API 的 ensureClassFile 和 transform 方法（PREVIEW）
     * JDK 24 PREVIEW 特性，需要 --enable-preview
     * 演示类文件的转换操作
     */
    @Test
    public void testClassFileTransform_Preview() throws Exception {
        // JDK 24 PREVIEW 特性，需要 --enable-preview
        // 确保目标类文件是可解析的类文件
        String className = this.getClass().getName();
        Class<?> clazz = this.getClass();

...
public class HiddenClassTest {
    ...
    public void testDefineHiddenClass() throws Exception {
        ...
        // 参数: 字节码、是否初始化、类选项(Lookup.ClassOption.NESTMATE 表示作为宿主的巢成员)
        Class<?> hiddenClass = lookup.defineHiddenClass(classBytes, true, Lookup.ClassOption.NESTMATE).lookupClass();
    ...
    public void testHiddenClassAnonymous() throws Exception {
        ...
        System.out.println("隐藏类是否为合成类: " + hiddenClass.isSynthetic());
        System.out.println("--- 分割线 ---");
    }
     ...
     * 使用类加载器读取 .class 文件
     *
     * 注意: 模板类必须是独立的顶层类(HiddenClassHelper),
     * 不能是嵌套类, 因为嵌套类的字节码带有 InnerClasses 属性,
     * 声明了与宿主类的嵌套关系, 作为隐藏类加载时会抛出
     * IncompatibleClassChangeError(与隐藏类名不匹配)。
     *

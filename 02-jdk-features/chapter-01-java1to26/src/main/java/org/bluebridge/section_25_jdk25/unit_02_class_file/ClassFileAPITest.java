package org.bluebridge.section_25_jdk25.unit_02_class_file;
...
public class ClassFileAPITest {
    ...
    public void testBuildClass() {
        ...
        byte[] classBytes = ClassFile.of().build(classDesc, clb -> {
            ...
            clb.withMethod("main", MethodTypeDesc.of(ClassDesc.ofDescriptor("V"),
                    ClassDesc.of("java.lang.String").arrayType()), AccessFlag.PUBLIC.mask() | AccessFlag.STATIC.mask(),
            ...
            clb.withMethod("add", MethodTypeDesc.of(ClassDesc.ofDescriptor("I"),
                    ClassDesc.ofDescriptor("I"), ClassDesc.ofDescriptor("I")),
     ...
     */
    @Test
    public void testParseMethodBytecode() throws IOException {
        // 获取当前类的字节码
        String className = this.getClass().getName();
        byte[] classBytes = this.getClass().getResourceAsStream(
                "/" + className.replace('.', '/') + ".class")
     ...
     */
    @Test
    public void testTransformClass() throws IOException {
        // 获取当前类的字节码
        String className = this.getClass().getName();
        byte[] classBytes = this.getClass().getResourceAsStream(
                "/" + className.replace('.', '/') + ".class")
        ...
        // 使用 ClassFile API 转写类文件(基于解析的模型, 原样输出所有元素)
        byte[] transformedBytes = ClassFile.of().transformClass(classModel, ClassTransform.ACCEPT_ALL);
     ...
     */
    @Test
    public void testReadConstantPool() throws IOException {
        // 获取当前类的字节码
        String className = this.getClass().getName();
        byte[] classBytes = this.getClass().getResourceAsStream(
                "/" + className.replace('.', '/') + ".class")
        ...
        System.out.println("  常量池条目数: " + constantPool.size());
        System.out.println("--------------------------------------");
     ...
     */
    @Test
    public void testVerifyClassFile() throws IOException {
        // 获取当前类的字节码
        String className = this.getClass().getName();
        byte[] classBytes = this.getClass().getResourceAsStream(
                "/" + className.replace('.', '/') + ".class")

 ...
 *
 * 1. 实现克隆需要做两件事
 *    - 实现 Cloneable 接口，这个接口中没有任何方法，只是一个标记接口
 *    - 需要重写 Object 类的 clone() 方法，调用 super().clone() 方法实现克隆
 * 2. 浅克隆：复制一个对象的复本
 *    - 对于基本数据类型，如：int，long，float 等：复制值
 *    - 对于复合数据类型：如数组，复制数组地址
 *    - 对于对象变量：复制对象的 reference
 * 3. 浅克隆深入之克隆一个 String 类型数据
 *    相当于 1 个 String 内存空间有两个引用，当修改其中的一个值的时候，会新分配一块内存用来保存新的值，这个引用指向新的内存空间，原来
 *    的 String 因为还存在指向他的引用，所以不会被回收，这样，虽然是复制的引用，但是修改值的时候，并没有改变被复制对象的值。
 *
 * @author lingwh

...
public class BitWiseRealBitFieldTest {
    ...
    public static void main(String[] args) {
        ...
        // ---------- 测试读取 setBitField 读取某一位 ----------
        int flagBit = BitWiseRealBitField.getBitField(value, bitOffset);
        System.out.printf("获取 value 中第 %d 位 = %d\n", bitOffset, flagBit, BitWiseRealBitField.showMemoryLayout(value));
        System.out.println("------------------------------");

        // ---------- 测试读取 setBitField 读取某一范围内的位 ----------
        int flagsBit = BitWiseRealBitField.getBitField(value, 1, 3);
        ...
        // ---------- 测试写入 setBitField 某一位----------
        int writeValue = 1;
        int newValue = BitWiseRealBitField.setBitField(value, bitOffset, writeValue);
        ...
        // 回读校验，同样格式
        int readNewValue = BitWiseRealBitField.getBitField(newValue, bitOffset);
        System.out.printf("回读 value 中第 %d 位, 十进制 = %d, 二进制 = %s\n",
                bitOffset, readNewValue, BitWiseRealBitField.showMemoryLayout(newValue));

        // ---------- 测试写入 setBitField 某一范围内的位----------
        // 准备写入的值
        writeValue = 0b111;
        newValue = BitWiseRealBitField.setBitField(value, bitOffset, bitCount, writeValue);
        ...
        // 回读校验，同样格式
        readNewValue = BitWiseRealBitField.getBitField(newValue, bitOffset, bitCount);

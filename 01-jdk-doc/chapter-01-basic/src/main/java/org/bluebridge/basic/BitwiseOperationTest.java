...
public class BitwiseOperationTest {
    ...
    /**
     * 测试位运算的左右移位 1.整型数据(包括正数和负数)支持左右移位，浮点类型数据不支持左右移位 2.如果向右移位不能整除，则向下取整，如下面的 j = 17 3.左移和右移分为有符号和无符号
     * 有符号移位：运算规则：在一定范围内，数据每向左移动一位，相当于原数据*2。（正数、负数都适用） 无符号移位：运算规则：往右移动后，左边空出来的位直接补0。（正数、负数都适用）
     */
    @Test

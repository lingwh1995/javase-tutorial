...
public class VoteManager {
    ...
    /**
     * 记录当前每个用户对应的状态处理对象，每个用户当前的状态是不同的 Map<String,VoteState> 对应
     * Map<用户名称，当前对应的状态处理对象>
     * 如果通过数据库管理状态，则上下文中不用再使用 mapState 来保存每一个用户对应的状态了
     */
    ...
    /**
     * 投票 @Version 3.0 将状态保存在数据库中，使用根据状态的全路径名，使用反射创建对象
     *
     * @param user     投票人，为了简单，就是用户名称
     * @param voteItem 投票的选项
     */

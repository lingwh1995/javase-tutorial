...
/**
 * 客户端不应该依赖它不需要的接口，即一个类对另一个类的依赖应该建立在最小的接口上 A2就是客户端 A2通过Interface1A,InterfaceC依赖B2类
 * 接口隔离后，interface2A只能调用operator1() interface2B只能调用operator2()和operator3()
...
/**
 * 客户端不应该依赖它不需要的接口，即一个类对另一个类的依赖应该建立在最小的接口上 C2就是客户端 C2通过Interface2A,Interface2B依赖D2类
 * 接口隔离后，interface2A只能调用operator1() interface2B只能调用operator2()和operator3()
 * interface2C只能调用operator4()和operator5()
 */
class C2 {
